...
func logSystemCaCerts() {
    // Get a list of the system CA certificates
    pool, err := x509.SystemCertPool()
    if err != nil {
        log.Printf("[ERROR] Failed to get system CA certificates: %s", err)
        return
    }
    certSubjectsInPool := pool.Subjects()
    // Print out each cert subject in the pool.
    log.Printf("[TRACE] System Cert Pool contains %d certificates", len(certSubjectsInPool))
    for i, subject := range certSubjectsInPool {
        log.Printf("%d:\n%s\n", i, string(subject))
    }
}
